...
class RecordTimerEntry(timer.TimerEntry, object):
    ...
    def __repr__(self):
        if not self.disabled:
            return 'RecordTimerEntry(name=%s, begin=%s, serviceref=%s, justplay=%s, isAutoTimer=%s)' % (self.name,
             ctime(self.begin),
             self.service_ref,
             self.justplay,
             self.isAutoTimer)
        else:
            return 'RecordTimerEntry(name=%s, begin=%s, serviceref=%s, justplay=%s, isAutoTimer=%s, Disabled)' % (self.name,
             ctime(self.begin),
             self.service_ref,
             self.justplay,
             self.isAutoTimer)
